package zerox06.dedicatedlanpinger.network;
...
public class BetterLanPinger extends Thread {
    ...
    public BetterLanPinger(Supplier<String> MOTDSupplier, String serverPort) throws IOException {
        ...
        setDaemon(true);
        setUncaughtExceptionHandler(new UncaughtExceptionLogger(DedicatedLanPingerMod.LOGGER));
    ...
    public void run() {
        while(!this.isInterrupted() && this.running) {
            try {
                ...
                DedicatedLanPingerMod.LOGGER.warn("Exception in Lan Pinger Thread: {}", e.getMessage());
                break;
            }
